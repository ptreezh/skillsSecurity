...
import { lowerThanOrEqual, parseVersion, } from "@nomicfoundation/hardhat-utils/fast-semver";
/**
...
export function instrumentSolidityFileForCompilationJob({ compilationJobSolcVersion, sourceName, fileContent, coverageLibraryPath, }) {
    ...
    assertHardhatInvariant(parsedLatestSupportedVersion !== undefined, `Invalid latest supported solidity version: ${latestSupportedVersion}`);
    ...
    assertHardhatInvariant(parsedInstrumentationVersion !== undefined, `Invalid solc version: ${instrumentationVersion}`);
    if (!lowerThanOrEqual(parsedInstrumentationVersion, parsedLatestSupportedVersion)) {
