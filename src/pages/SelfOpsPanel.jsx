...
export default function SelfOpsPanel({ user, deployerStats }) {
  ...
  const tierLabel = t(
    `deployerDashboard.tiers.${["bronze", "silver", "gold"][deployerStats?.tier ?? 0]}.name`,
  );
  ...
  const { data: promotionResult, loading: promotionLoading } = usePolling(
    // 晋升榜：chain1 真链声誉排行（v2），适配 Leaderboard 子组件字段（domain/totalUsers/tier/trend）
    async () => {
      try {
        ...
        return {
          leaderboard: users.map((u) => ({
            rank: u.rank,
            domain: u.address, // 推广者标识 = 链上地址
            totalUsers: u.skillsCreated, // 推广成果 = 参与技能数
            tier: 0,
            trend: 0,
          })),
        };
  ...
  useEffect(() => {
    if (promotionResult)
      setPromotionData((p) => ({
        ...p,
        leaderboard: promotionResult.leaderboard || [],
      }));
  }, [promotionResult]);
  ...
  useEffect(() => {
    if (governanceResult?.proposals)
      setGovernanceData((p) => ({
        ...p,
        proposals: governanceResult.proposals || [],
      }));
  }, [governanceResult]);
      ...
      <div className="self-ops-content">
        {activeTab === "revenue" && (
          <section className="animate-fade-in">
            <h3 className="self-ops-section-title">
              {t("selfOps.revenueOverview")}
            </h3>
        ...
        {activeTab === "promotion" && (
          <section className="animate-fade-in">
            <h3 className="self-ops-section-title">
              {t("selfOps.promotionLeaderboard")}
            </h3>
            <PromotionBarChart
        ...
        {activeTab === "governance" && (
          <section className="animate-fade-in">
            ...
            <GovernancePieChart
              proposal={
                selectedProposal ||
                governanceData.proposals[0] || { forVotes: 0, againstVotes: 0 }
              }
                ...
                governanceData.proposals.map((proposal) => (
                  ...
                  >
                    ...
                    <span className="proposal-votes">
                      {t("selfOps.votesFor", {
                        count: Number(proposal.forVotes || 0),
                      })}{" "}
                      /{" "}
                      {t("selfOps.votesAgainst", {
                        count: Number(proposal.againstVotes || 0),
                      })}
                    </span>
          ...
          <section className="animate-fade-in">
            <div className="health-actions">
              ...
              >
                ...
                <span className="health-action-name">
                  {t("selfOps.healthReports.bug")}
                </span>
                <span className="health-action-reward">+50 RP</span>
              </button>
              <button
              ...
              >
                <span className="health-action-icon">📊</span>
                <span className="health-action-name">
                  {t("selfOps.healthReports.status")}
                </span>
                <span className="health-action-reward">+10 RP</span>
              </button>
              <button
              ...
              >
                <span className="health-action-icon">⚡</span>
                <span className="health-action-name">
                  {t("selfOps.healthReports.stress")}
                </span>
                <span className="health-action-reward">+100 RP</span>
              </button>
            </div>
            <div className="health-stats">
              <span className="health-stats-label">
                {t("selfOps.monthlyRemaining")}
              </span>
              <span className="health-stats-value">
                {Math.max(
                  0,
                  (healthData.stats?.maxMonthly || 10) -
                    (healthData.stats?.monthlyCount || 0),
                )}{" "}
                / {healthData.stats?.maxMonthly || 10}
              </span>
            </div>
