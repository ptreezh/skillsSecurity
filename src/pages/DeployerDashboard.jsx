...
export default function DeployerDashboard({ user }) {
  ...
  const tierConfig = {
    0: {
      name: t("deployerDashboard.tiers.bronze.name"),
      key: "bronze",
      benefit: t("deployerDashboard.tiers.bronze.benefit"),
    },
    1: {
      name: t("deployerDashboard.tiers.silver.name"),
      key: "silver",
      benefit: t("deployerDashboard.tiers.silver.benefit"),
    },
    2: {
      name: t("deployerDashboard.tiers.gold.name"),
      key: "gold",
      benefit: t("deployerDashboard.tiers.gold.benefit"),
    },
  };
  ...
  useEffect(() => {
    async function fetchDeployerData() {
      ...
      try {
        ...
        setReferralLink(
          `https://agent-skills.xyz/ref/${user.address.slice(2, 10).toLowerCase()}`,
        );
      } catch (err) {
  ...
  const getProgressToNextTier = () => {
    ...
    const progress =
      ((stats.totalUsers - currentThreshold) /
            ...
            <div className="tier-preview">
              ...
              <div className="tier-list">
                {Object.entries(tierConfig).map(([tier, config]) => (
                  <div
                    key={tier}
                    className={`tier-item tier-item-${config.key}`}
                  >
                    <span className={`tier-name tier-name-${config.key}`}>
                      {config.name}
                    </span>
                    <span>
        ...
        <div className="stats-grid">
          <div className="stat-card">
            <div className="stat-label">
              {t("deployerDashboard.stats.totalUsers")}
            </div>
            <div className="stat-value">{stats.totalUsers}</div>
          </div>
          <div className="stat-card">
            <div className="stat-label">
              {t("deployerDashboard.stats.activeUsers")}
            </div>
            <div className="stat-value">{stats.activeUsers}</div>
          </div>
          <div className="stat-card stat-highlight">
            <div className="stat-label">
              {t("deployerDashboard.stats.totalRewards")}
            </div>
            <div className="stat-value">
              {stats.totalRewards.toLocaleString()}
            </div>
            <div className="stat-unit">RP</div>
          </div>
          <div className="stat-card">
            <div className="stat-label">
              {t("deployerDashboard.stats.pendingRewards")}
            </div>
        ...
        <div className="tier-progress">
          ...
          <div className="progress-info">
            <span className={`current-tier tier-text-${tierKey}`}>
              {tierInfo.name} · {stats.totalUsers}{" "}
              {t("deployerDashboard.users")}
            </span>
            {progress.nextTier ? (
              <span className="next-tier">
                {t("deployerDashboard.toNextTier", {
                  tier: progress.nextTier.name,
                  count: progress.next - progress.current,
                })}
              </span>
            ) : (
